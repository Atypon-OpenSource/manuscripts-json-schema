...
(async function() {
  ...
  const types = exportedTypes.reduce(
    (acc, type) => {
...
(function (ObjectTypes) {
  ...
  await appendToDistFile(
    'lookup.js',
    'es',
    `export const manuscriptIDTypes = new Set(${JSON.stringify(
      types.manuscriptIDTypes
    )});
export const containerIDTypes = new Set(${JSON.stringify(
      types.containerIDTypes
    )});
